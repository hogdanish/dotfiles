...
Applies to every script, function, abbreviation or command wrapper that a human will run — `.fish`,
`.sh`, `.bash`, `.zsh`, and one-off commands you hand the user. The **`gum` skill is authoritative**;
this is the always-on trigger.
...
indicator, a banner, a table, a rendered document, or status/error output. Read
`references/commands.md` for any gum command you have not written before, and `references/recipes.md`
before composing more than one call.

Do **not** skip it because the script "only has one prompt". The defaults are the trap — see
`--default` below.
...
| Instead of | Use |
| --- | --- |
| `read -p` / `read -P` | `gum input` (`gum write` for multi-line) |
...
1. **Guard it**: `type -q gum` (fish) / `command -v gum` (bash) before the first gum call, with a
...
4. **Never `gum input --password` for a credential 1Password holds** — `op run` / `op://` first
   (`.claude/rules/security.md`). If a passphrase must be typed, pipe it into the consumer, never a
   variable.
