---
name: fish
description: "Fish 4.8.1 for this dotfiles repo: house style, language, builtins, variables, load order, functions, completions, abbreviations, key bindings, prompt, theming, integrations, startup caching, and verified caveats. Load before editing any .fish file or ~/.config/fish, translating shell code to fish, diagnosing fish behavior or startup cost, or changing fzf, zoxide, atuin, starship, or grc integration. Owns only the fish side of cross-tool config."
---
...
`_` prefix is this config's ordering mechanism. Load order as of 2026-08-07 — **eighteen** snippets:
...
| 6 | `cloudflare.fish` | `CLOUDFLARE_ACCOUNT_ID`, wrangler telemetry off, and ⚠ `CLOUDFLARE_AUTH_USE_KEYRING` — without it `wrangler login` writes a plaintext OAuth token to `~/.config/.wrangler/`, inside this repo |
| 7 | `colours.fish` | **all** colour: the `$theme_*` palette, `LESS*`, `BAT_*`, `MANPAGER`, `LS_COLORS`/`EZA_COLORS`, every `fish_color_*` |
| 8 | `fzf.fish` | `FZF_*` — ⚠ must precede `tools.fish` |
| 9 | `ghostty.fish` | terminal shell integration, `COLORTERM` |
| 10 | `git.fish` | `GIT_CONFIG_GLOBAL`, `GIT_CONFIG_SYSTEM`, `GIT_PAGER` |
| 11 | `gum.fish` | ~24 `GUM_*` laramie accents + `GUM_FORMAT_THEME` — ⚠ *not* interactive-guarded, scripts need it (`gum` skill) |
| 12 | `java.fish` | `$JAVA_HOME` — ⚠ hardcoded, not `/usr/libexec/java_home` (a 5.7 ms fork) |
| 13 | `keybindings.fish` | `bind` statements — ⚠ loses any key a tool init later binds |
| 14 | `op.fish` | 1Password agent socket, Claude Code and Codex environment ids |
| 15 | `rust.fish` | rustup/Cargo XDG homes, sccache cache, and rustup's keg-only paths |
| 16 | `tools.fish` | cached inits: fzf, zoxide, starship, atuin |
| 17 | `uv.fish` | `~/.local/bin` (uv's tool-shim dir) on `$PATH` — ⚠ must follow `brew.fish`, same as `bun.fish` |
| 18 | `xdg-apps.fish` | per-tool XDG env incl. `GLAMOUR_STYLE` — ⚠ sorts last, so nothing earlier may read it |
