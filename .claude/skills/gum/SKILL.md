---
name: gum
description: The gum CLI (0.17.0) — the toolkit for every interactive or formatted shell script on this machine. Covers all 13 commands (choose, filter, confirm, input, write, file, table, pager, spin, style, join, format, log) with every flag, default, GUM_* environment variable and enum; the scripting contract (TUI on stderr / result on stdout, exit codes 0/1/124/130, tty requirements, stdin short-circuits); layout composition with style + join; markdown rendering through the vendored glamour (gum format is the only glamour surface here — no glow binary, and gum reads GUM_FORMAT_THEME, never GLAMOUR_STYLE); the live laramie theming in ~/.config/fish/conf.d/gum.fish; and fish-specific idioms where command-substitution splitting breaks multi-line gum output.
when_to_use: Load before writing or editing ANY shell script, fish function, abbreviation or command wrapper that prompts the user, asks a yes/no question, shows a menu, reports progress, prints a banner or table, or formats terminal output — and before answering "how should this script ask/print X". Also for gum theming, glamour/markdown rendering in the terminal, and any exit-code, tty or piping problem in a gum-using script. Prefer gum over bare read/echo/printf/select for anything user-facing. Boundary — this owns gum usage and gum output styling; fish syntax and ~/.config/fish belong to the `fish` skill, package installation to `brewfile`, and prompted credentials to `auth` (which usually means: do not prompt, use op).
