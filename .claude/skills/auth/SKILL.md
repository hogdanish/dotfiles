---
name: auth
description: Authentication, secrets and credential management on this machine — 1Password as the root of trust (`op` CLI, secret references `op://`, `op run`/`inject`/`read`, Environments, shell plugins, service accounts), the 1Password SSH agent (`agent.toml`, `IdentityAgent`, SSH bookmarks, `op-ssh-sign` commit signing), the GnuPG chain (gpg-agent → pinentry-touchid → pinentry-mac → macOS keychain), and every Touch ID surface (1Password unlock, CLI biometric unlock, SSH authorization prompts, `pam_tid.so` for sudo). Also owns the no-plaintext-secrets policy and the migration of `~/.config/fish/conf.d/secrets.fish` to `op://` references.
when_to_use: Load before touching anything to do with secrets, API keys, tokens, `.env` files, SSH keys or config, `~/.ssh`, `~/.gnupg`, GPG or commit signing, 1Password/`op`, keychain, Touch ID, sudo authentication, or PAM — and before writing any config that consumes a credential (an MCP server `env` block, a `conf.d/*.fish` that exports a token, a script that needs an API key). Also load when asked "how do I stop typing my password", "where should this key live", or when auditing for plaintext secrets. Boundary — this owns credential storage/retrieval and authentication; fish syntax belongs to the `fish` skill and package installation to `brewfile`, though both are used here.
