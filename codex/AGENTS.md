...
- When prompting the user with options in planning mode (i.e. `/plan`), indicate which option you personally recommend and why as concisely as possible.
...
- Treat this file as a router. Keep detailed, task-specific guidance in skills or references so it is loaded only when relevant.
- User-authored Codex configuration lives in `~/.config/codex/`; `~/.codex/AGENTS.md` and `~/.codex/config.toml` are compatibility symlinks into that tracked directory.
- User-authored skills live in `~/.config/codex/skills/` and are linked individually into `~/.agents/skills/`. Vendor skills belong in plugins or installers, not the dotfiles repo.
- Before shell-heavy work or recommending a tool, read `~/.config/codex/rules/toolbox.md`. The Brewfile remains the complete software inventory.
- Before using `cc-notify`, read `~/.config/codex/rules/notify.md`; default to silence and pair every `hold` with `clear`.
- For substantial drafting or revision, use the `prose` skill. For Godot/GDScript or OrbStack work, use the matching skill.
- A surprising failure or disproved assumption is a documentation defect. Update the owning rule, skill, reference, or audit in the same change when it is in scope.
