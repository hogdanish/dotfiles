...
name: simple-english
description: |
  Rewrite or draft text in plain, layman-readable English with the discipline of ASD-STE100
  Simplified Technical English: sentences under 20 words, active voice, simple tenses, one
  word one meaning, condition before command, every technical term defined at first use, no
  AI slop. OPT-IN ONLY on this machine. Load it when the user asks for it by name or by
  register — "simple english", "STE", "Simplified Technical English", "ASD-STE100", "plain
  english", "layman's terms", "explain it simply", "no jargon", "de-slop", "make this
  readable", "write for non-native readers", "write this like a Boeing manual" — or when the
  user asks for text that must survive translation. Do NOT load it for ordinary writing:
  a README, a commit message, a code comment, a docstring, release notes, an error string,
  or a normal reply stays in the house voice unless the user names this register. Applying
  it unasked is the failure mode, not a bonus.
license: MIT
...
<!-- LOCAL OVERRIDE START — authored here, not upstream.
     Everything below the END marker is github.com/AminBlg/SimpleEnglish v2.0.0, verbatim
     (MIT, © AminBlg). The frontmatter description and this block are the only local edits.
     Do NOT hand-edit the rest: scripts/simple-english-sync.sh re-fetches and diffs it. -->

## Local policy (this machine)

Two rules override the upstream text that follows. They lose to an explicit instruction in the
current request, and to nothing else.

1. **Plain mode always. Never Strict.** Ignore the Strict row of the mode table below and every
   rule marked (S). Do not read `references/strict-vocabulary.md`. This holds even when the
   request says "STE" or "ASD-STE100" — here those words name this skill, not a compliance
   target. Switch to Strict only if the request says "strict" outright, or asks for real
   ASD-STE100 dictionary compliance for a document that will be audited.
2. **The document only, not the reply.** Apply the register to the text you are asked to write
   or rewrite. Answer the user in the normal voice, unless they ask for the reply in Simple
   English too.
