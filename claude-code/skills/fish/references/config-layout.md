...
regardless of name. Verified: with `aaa-vendor.fish` in `vendor_conf.d` and `zlower.fish` in the
user `conf.d`, `zlower` ran first.
...
sysconf `conf.d` and a `vendor_conf.d` produced exactly one line of output — the user's. The other
two were skipped by the `contains`/`continue` guard.
...
variables. So `fish --no-config -c 'reload'` fails with `Unknown command: reload` — your own
functions do not autoload — and `$PATH` is missing everything `/etc/paths` would have contributed.
...
- A numeric prefix beats `_`. Nothing currently needs to run before `_init.fish`, so
  **do not introduce numeric prefixes** — extend the `_` set instead (see
  [style-guide.md](style-guide.md) §2).
...
`config.fish` is for **overriding a snippet you cannot edit** — a vendor or sysconf file. That is
the only reason it sources last. Nothing else goes there.
...
- Completion for command `git` → the first `git.fish` in `$fish_complete_path`, loaded the first
  time completion for `git` is requested.
- ⚠ **The filename must equal the function name.** A file whose name matches but which does not
  define that function stops the search: *"If a file of the right name doesn't define the function,
  fish will not read other autoload files, instead it will go on to try builtins and finally
  commands."* That is the documented way to mask a lower-precedence function.
...
2. The `*/` glob is safe when there are no subdirectories — `functions/` currently has none.
   Normally a failed glob aborts the command with status 124, but *"There are exceptions, namely
   `set` and `path`, overriding variables in overrides, `count` and `for`. Their globs will instead
   expand to zero arguments"*. `path resolve` is on that list, so this expands to nothing and the
   assignment is a no-op. Verified: no error, status 0. Change `path resolve` to any other command
   and startup breaks.
3. ⚠ **Failure mode: the glob runs once, at startup.** Creating `functions/macos/` in a live shell
   does not make it searchable — `exec fish` (or the `refresh` abbreviation) is required. If a
   brand-new namespaced function reports `Unknown command`, restart before debugging anything else.
...
formatting, with no comments and no `--description` discipline — they silently overwrite
hand-authored config. `set -U` is already banned by [style-guide.md](style-guide.md) §3, and
`fish_config theme save` is a `set -U` with extra steps.

**Author the file directly.** If you do reach for `funced` — legitimate for iterating on a prompt in
a real terminal, where nothing else gives you live feedback:
...
`$GHOSTTY_RESOURCES_DIR/shell-integration/fish/vendor_conf.d/ghostty-shell-integration.fish`, and
fish **does** scan it — Ghostty prepends `…/shell-integration` to `XDG_DATA_DIRS`, so step 1
computes `$__fish_vendor_confdirs` with that directory in it. The snippet's first act is to strip
that entry again, which is why the table above shows no existing `vendor_conf.d`: the measurement is
taken after the strip. Consequences, both verified:

- The manual `source` in `_shell.fish` makes the top-level shell load the snippet **twice** (step 5
  by hand, step 7 from `vendor_conf.d`). It is written to be re-entrant, so this is harmless.
...
| -------------------------------------- | ------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `~/.config/fish/conf.d/`               | sourced snippets, step 5                                                        | 18 files — see §7                                                                                                                                                                                                                                                                   |
...
`atuin uuid`". That was wrong — the fork is preemptable by satisfying atuin's own guard with
builtins, and removing it is where most of the second reduction came from. See
...
| File               | Owns                                                                                                                                                                           | Notable                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `_init.fish`       | XDG base dirs, `$PROJECTS`/`$DOTFILES`/`$CODEX_HOME`, `$FISH_THEMES_DIR`, the `functions/*/` + `completions/*/` autoload globs, core env (`PAGER`/`VISUAL`/`EDITOR`/`BROWSER`) | XDG defaults are conditional (`test -n "$X"; or set -gx …`) so an exported value wins. `CODEX_HOME` stays at `~/.codex` for the GUI app; authored files inside it link to `$XDG_CONFIG_HOME/codex`. ⚠ `CLAUDE_CONFIG_DIR` was exported here until 2026-09-12 and must NOT come back — Claude Code namespaces its macOS keychain item by that variable's hash, which hid the live login from clauth and from every other account manager; `~/.claude` is a symlink to `$XDG_STATE_HOME/claude` instead, so the storage is unchanged. One guarded `mkdir` via `path filter -vd` replaced unconditional calls. ⚠ `mkdir -p` with an empty list is an *error*, hence the `test -n "$missing"` guard. No `MANPATH` block — macOS derives it from `$PATH` via `MANPATH_MAP` and setting it explicitly would break that                                        |
...
| `clauth.fish`      | `CLAUTH_NO_API`, `CLAUTH_NO_COMPLETIONS`                                                                                                                                       | Added 2026-09-12. ⚠ Guarded with `command -q`, NOT `type -q`: `functions/wrappers/clauth.fish` shadows the name, so `type -q` is true whether or not the binary exists. Pins the clauth daemon's REST listener shut and suppresses the first-run completions prompt, since completions are installed deliberately. ⚠ The launchd agent inherits no fish environment, so `home/launchd/dev.uwuclxdy.clauth.plist` repeats `CLAUTH_NO_API` itself                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `op.fish`          | 1Password SSH agent socket, the Claude Code and Codex environment ids                                                                                                          | Both wrappers use the same development Environment without changing either agent's own login. ⚠ Never source `~/.config/op/plugins.sh` — POSIX shell, fails `fish -n`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `rust.fish`        | `$RUSTUP_HOME`, `$CARGO_HOME`, `$SCCACHE_DIR`, `$CARGO_HOME/bin` and Homebrew's keg-only rustup bin on `$PATH`                                                                 | Added 2026-08-07. Data belongs under `$XDG_DATA_HOME`; compiled-object cache belongs under `$XDG_CACHE_HOME`. The directories are created before `fish_add_path`, which otherwise skips missing entries. Homebrew's rustup keg supplies the rustc/Cargo proxies; `$CARGO_HOME/bin` is for `cargo install` output. Both paths append so language-level globals cannot shadow Homebrew or project tools. Cargo behaviour stays in tracked `cargo/config.toml`; no global `RUSTFLAGS` or shared target directory                                                                                                                                                                                                                                                                                                                                           |
...
| `uv.fish`          | `~/.local/bin` — uv's tool-shim directory — on `$PATH`                                                                                                                         | Added 2026-07-30. ⚠ Must sort **after** `brew.fish` for the same reason `bun.fish` does. `uv tool install` symlinks its shims into `~/.local/bin` (`uv tool dir --bin`, which resolves `$XDG_DATA_HOME/../bin`), and **no shell here had that directory on `$PATH`** — gdtoolkit was installed 2026-07-29 and `gdformat`/`gdlint` were unreachable in fish *and* zsh until this file landed. ⚠ The path is a literal: `uv tool dir --bin` is a fork, and this file is sourced by non-interactive shells too (`java.fish` makes the same trade). ⚠ `-a` so a uv shim cannot shadow a Homebrew binary; `mkdir` first because `fish_add_path` silently skips a nonexistent directory. No `UV_*` variables — uv is XDG-correct unaided and behaviour lives in `$XDG_CONFIG_HOME/uv/uv.toml`. Costs ~0.58 ms, essentially all of it the `fish_add_path` call |
...
| `wrappers/clauth.fish`                            | Injects `--settings <the tracked settings.json>` into `clauth start` only — that subcommand forwards unrecognised arguments to the `claude` it spawns, and there is deliberately no `settings.json` in the Claude config dir for it to inherit, because clauth rewrites that path on every account switch. Every other subcommand passes straight through. ⚠ Appended last, so a preceding *variadic* claude flag would swallow the path                                                                                                                                                                  |
...
| Path                      | State                                                                                                                                                                                                                                                   |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `completions/`            | Authored/generated local completions plus tracked symlinks to OrbStack/Homebrew-owned `docker.fish`, `kubectl.fish`, and `orbctl.fish`. ⚠ `fish_indent` follows symlinks, so formatter writes must skip those vendor targets; the pre-commit hook does. |
| `themes/laramie.fish`     | The palette: 23 named `$theme_*` colours, hex **with** a leading `#` (`set_color` accepts it). Source of truth                                                                                                                                          |
| `themes/laramie.theme`    | Generated by `claude-code/skills/fish/scripts/gen-fish-theme.fish` so `fish_config theme list/show` works. ⚠ Bare hex, **no** `#` — a `.theme` file is tokenised, so `#` starts a comment and the value is silently lost                                |
...
Halve-and-repeat with a scratch environment when the interaction is between files — this reproduces
a startup without touching the live config at all:
