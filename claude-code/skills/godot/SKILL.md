...
- **Versioning**: We typically run Godot **development builds** — APIs shift extremely quickly. Verify anything version-sensitive or obscure against current docs before implementing. Prefer Context7 (`/godotengine/godot-docs`) → web search when appropriate. Do NOT trust training data for 4.x features.
...
- **Verify lean, then stop** — a human checks most changes in ~30 s; don't out-spend that with MCP round-trips. Spend the cheapest tier that proves the change:
