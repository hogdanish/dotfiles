...
symlink into this repo), and it must stay ahead of every `Host` block for first-match-wins
ssh_config semantics. `~/.orbstack/ssh/config` is regenerated by OrbStack and marked do-not-edit.
...
Rosetta x86 error inside a machine:
`sudo dpkg --add-architecture amd64 && sudo apt install libc6:amd64`. Cloud-init debug:
`orb -m myvm cloud-init status --long`, or read `/var/log/cloud-init-output.log` on that machine.
